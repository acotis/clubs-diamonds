...
impl<N: Number, const C: usize> OrWriter<N, C> {
    pub fn new(length: usize, max_constant: Option<u128>) -> Self {
        ...
        Self {
            max_constant,
            children: Children::standard(CHILD_OF_OR, max_constant, OR, &initial_partition.state()),
    ...
    pub fn write(&mut self, dest: &mut [u8]) -> bool {
        loop {
            ...
            if self.partition.next() {
                self.children = Children::standard(CHILD_OF_OR, self.max_constant, OR, &self.partition.state());
                continue;
            }
