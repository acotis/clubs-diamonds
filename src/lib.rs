...
//!
//! Sometimes, it's useful to know more information about an expression than just that it met the search criterion. For example, you might want to search for expressions that have certain outputs for the input values 1, 2, 3, 4, and 5, but then to actually put the expression to use, you might need to know what its output is for an input of 6.
//!
...
//!
//! [`Verdict`] is also implemented for `Vec`. An empty `Vec` rejects the expression and a non-empty `Vec` accepts it. Just like with `Option`, the extra data is passed along with the accepted expression to any inspector or penalizer, and is included in the final results list, and just like with `Option`, this is done via the `Solution` struct. In the case of `Vec`, the entire non-empty `Vec` itself is what's put in the `data` field of that struct.
//!
